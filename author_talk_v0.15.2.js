...
function buildChatHistory(character, chat) {
    const usable = usableChatMessages(chat);
    const included = usable;
    const lines = included.map((message) => {
    ...
    return {
        ...
        searchable: usable.map((message) => message.data),
    };
}
...
function evaluateStandaloneAutoLore(entry, key, identity, searchableMessages, memoTexts, scanDepth, fullWord) {
...
function evaluateLoreViews(views, identity, searchableMessages, memos) {
    ...
    for (let pass = 0; pass < Math.max(1, views.length); pass++) {
        ...
        for (const view of views) {
            ...
            if (recursive && view.content.trim())
                recursiveDocuments.push({ text: view.content, source: "recursive" });
        }
        if (!changed)
            break;
    }
...
async function buildLoreViews(identity, searchableMessages, cbsEnvironment, memos) {
    if (!currentWorkspace)
...
async function buildWriterContext() {
    ...
    if (firstMessages.length === 0)
        firstMessages.push(processCbsReference("", cbsEnvironment, omitsUnsupportedSyntax("firstMessage")));
...
async function buildMemoTriggeredLoreBlock(identity, workspace, messages) {
    ...
    for (let index = 0; index < entries.length; index++) {
        ...
        if (!content || entry?.mode === "folder" || entry?.mode === "child" || entry?.alwaysActive)
            continue;
        const signature = loreSignature(entry);
        ...
        if (withoutMemo.active || !withMemo.active || requestAlreadyContainsLore(messages, content))
            continue;
...
async function ensureMemoReceiptPresent(state = memoReceiptState) {
...
async function ensureMemoReceiptObserver() {
    if (!mainDocument || memoReceiptObserver)
        return;
...
async function displayMemoReceipt(identity, block) {
    ...
    for (let attempt = 0; attempt < 8 && state === memoReceiptState; attempt++) {
        if (await ensureMemoReceiptPresent(state))
            return;
...
async function hasStoredActiveMemo() {
    try {
...
async function undoMemoActions(messageId) {
    ...
    if (!message?.actionUndo || message.actionState !== "applied")
        return;
    if (message.actionUndo.memoFolders?.length)
        currentWorkspace.memoFolders = safeClone(message.actionUndo.memoFolders);
...
function streamChunkText(chunk, accumulated) {
    if (typeof chunk === "string")
        return accumulated + chunk;
    if (chunk instanceof Uint8Array)
        return accumulated + new TextDecoder().decode(chunk, { stream: true });
    if (chunk && typeof chunk === "object") {
        ...
        if (typeof chunk.content === "string")
            return accumulated + chunk.content;
    }
    return accumulated;
}
...
function renderCbsAwareText(value, warnings) {
    if (warnings.length === 0)
...
function renderContextText(value, fallback, warnings) {
...
function renderContextTabLegacy() {
    if (isRefreshingContext)
...
async function handleClick(event) {
    ...
    if (action === "prev-first-message" || action === "next-first-message") {
        ...
        if (fmDetails) {
            ...
            if (tokenBadge)
                tokenBadge.outerHTML = renderTokenBadge(currentContext.tokenEstimates.firstMessage, currentContext.rawTokenEstimates.firstMessage);
        ...
        if (tokenStat)
            tokenStat.textContent = `참고 자료 약 ${currentContext.referenceTokens.toLocaleString()} 토큰`;
        return;
    }
    ...
    if (action === "set-all-unsupported-syntax" && currentContext) {
        ...
        for (const key of CONTEXT_TOGGLE_KEYS)
            settings.omitUnsupportedSyntax[key] = checked;
        for (const entry of currentContext.loreEntries)
            settings.omitUnsupportedSyntax[loreUnsupportedSyntaxKey(entry.key)] = checked;
        await saveSettings();
        currentContext = await buildWriterContext();
        renderPreservingPanelScroll();
        return;
...
async function initialize() {
    ...
    await Risuai.onUnload(async () => {
        ...
        await saveCurrentWorkspace();
        if (memoReplacerReady)
            await Risuai.removeRisuReplacer("beforeRequest", memoReplacer);
        memoReceiptState = null;
